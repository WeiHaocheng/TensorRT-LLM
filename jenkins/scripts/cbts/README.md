...
| `ModelingV2Rule` | `modelingv2only` | `tensorrt_llm/_torch/modeling_v2/**` (excl. `.md`) |
...
| `testsonly` | Multiple rules from the testsonly family fired (`waiveonly`, `testdefonly`, `testlistonly`, `autodeployonly`, `visualgenonly`, `specdeconly`, `modelingv2only`, `agentflowonly`, `openengineonly`); their narrows union. |
